...
用户当前策略是过滤掉 **70MB 以下**的小录像，因此 `filtering_threshold` 应继续按用户实际配置保留，而不是强制改成 0。
...
否则会改变用户明确要求的 70MB 过滤语义，并导致大量很小的片段进入 B站投稿流程。

### 风险必须知道

如果一场真正有价值的直播因为很短、断流或主播马上下播，最终单文件不足 70MB，它仍会按用户配置被永久过滤掉。
...
7. **没有修改用户明确配置的 `postprocessor=rm` 成功后删除语义**。
