...
def _modify_download(download: str) -> str:
    ...
    live_pattern = re.compile(r"(Ok\(LiveStatus::Live\s*\{.*?\}\)\s*=>\s*\{.*?retry_count\s*=\s*0;)", re.S)
    live_match = live_pattern.search(download)
    if not live_match:
        raise ModifyError("live retry branch not found")
    block = live_match.group(1)
    if "last_check_offline" not in block:
        block_new = block.replace("retry_count = 0;", "retry_count = 0;\n                    last_check_offline = false;", 1)
